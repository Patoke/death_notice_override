...
namespace jm {
    ...
    namespace detail {
        ...
        _FORCEINLINE constexpr std::size_t _buffer_size()
        {
            return ((Size / 16) + (Size % 16 != 0)) * 2;
        }

        template<std::uint32_t Seed>
        _FORCEINLINE constexpr std::uint32_t key4() noexcept
        {
        ...
        _FORCEINLINE constexpr std::uint64_t key8()
        {
        ...
        _FORCEINLINE constexpr std::uint64_t
        ...
        _FORCEINLINE std::uint64_t load_from_reg(std::uint64_t value) noexcept
        {
...
#ifndef JM__DISABLE_AVX_INTRINSICS
    ...
    public:
        ...
        template<class L>
        _FORCEINLINE xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>) noexcept
            : _storage{ ::jm::detail::load_from_reg((std::integral_constant<std::uint64_t, detail::load_xored_str8<Size>(Keys, Indices, l())>::value))... }
        {}

        _FORCEINLINE constexpr size_type size() const noexcept
        {
            return Size - 1;
        }

        _FORCEINLINE void crypt() noexcept
        {
...
#endif
#elif !defined(JM__DISABLE_AVX_INTRINSICS)
        ...
        _FORCEINLINE const_pointer get() const noexcept
        {
            return reinterpret_cast<const_pointer>(_storage);
        }

        _FORCEINLINE pointer get() noexcept
        {
            return reinterpret_cast<pointer>(_storage);
        }

        _FORCEINLINE pointer crypt_get() noexcept
        {
